...
class DashboardController extends Controller
{
    ...
    {
        $totalPendaftar = DataRegistrasi::where('status','>=',3)->whereIn('id_jalur',[2,6,7,8])->count();
        $pendaftarSudahDiproses = DataRegistrasi::where('status', '>', 3)->whereIn('id_jalur',[2,6,7,8])->count();
        $pendaftarHarusDiproses = DataRegistrasi::where('status', 3)->whereIn('id_jalur',[2,6,7,8])->count();
