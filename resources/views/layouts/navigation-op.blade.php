...
<nav x-data="{ open: false }" class="sticky top-0 z-50 bg-tertiary mx-auto w-full right-0 left-0">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class=" hidden lg:hidden h-64 overflow-y-auto">
        <div class=" pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route($redirectUrls)" :active="request()->routeIs($redirectUrls)">
                {{ __('Data Pendaftar') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route($redirectUrlsyarat)" :active="request()->routeIs($redirectUrlsyarat)">
                {{ __('Konfigurasi Persyaratan') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route($redirectUrljalur)" :active="request()->routeIs($redirectUrljalur)">
                {{ __('Konfigurasi Jalur') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route($redirectUrlpot)" :active="request()->routeIs($redirectUrlpot)">
                {{ __('Konfigurasi Pekerjaan Orang Tua') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route($redirectUrltes)" :active="request()->routeIs($redirectUrltes)">
