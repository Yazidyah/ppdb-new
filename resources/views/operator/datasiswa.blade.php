                    ...
                    <tbody>
                        @forelse ($data as $siswa)
                            <tr class="hover:bg-gray-200 transition duration-200 cursor-pointer">
                                ...
                                <td scope="col" class="{{ $tdClass }}">
                                    {{ optional(optional($siswa->dataRegistrasi)->dataTes)->pluck('id_jadwal_tes')->isNotEmpty() ? optional($siswa->dataRegistrasi->dataTes)->pluck('id_jadwal_tes')->join(' / ') : '-' }}
                                </td>
                                <td scope="col" class="{{ $tdClass }}">
                                    {{ $siswa->status_label ?? '-' }}
                                </td>
        ...
        <script>
            ...
            function toggleSortOrder(column) {
                ...
                if (currentSortBy === column) {
                    ...
                    document.querySelector('input[name="sort_order"]').value = defaultDescColumns.includes(column) ? 'desc' :
                        'asc';
                }
