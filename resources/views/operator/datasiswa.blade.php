                    ...
                    <form method="GET" action="{{ route('operator.datasiswa') }}" class="mb-4 flex justify-between"
                        ...
                        <div>
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Cari Nama atau NISN" class="px-4 py-2 border rounded-lg">
                        ...
                        <div>
                            <select name="filter" class="px-4 py-2 border rounded-lg"
                                ...
                                <option value="" {{ !request('filter') ? 'selected' : '' }}>Tidak ada filter data
                                </option>
                                <option value="L" {{ request('filter') == 'L' ? 'selected' : '' }}>Laki-laki
                                </option>
                                <option value="P" {{ request('filter') == 'P' ? 'selected' : '' }}>Perempuan
                                </option>
                                <option value="NEGERI" {{ request('filter') == 'NEGERI' ? 'selected' : '' }}>Sekolah
                                    Negeri</option>
                                <option value="SWASTA" {{ request('filter') == 'SWASTA' ? 'selected' : '' }}>Sekolah
                                    Swasta</option>
                                <option value="0" {{ request('filter') == '0' ? 'selected' : '' }}>Belum diproses
                                </option>
                                <option value="1" {{ request('filter') == '1' ? 'selected' : '' }}>Lulus</option>
                                <option value="2" {{ request('filter') == '2' ? 'selected' : '' }}>Tidak Lulus
                                </option>
                            </select>
                            ...
                            <tbody>
                                @forelse ($data as $siswa)
                                    <tr onclick="window.location.href='{{ route('operator.datasiswa-detail', $siswa->id_calon_siswa) }}'"
                                        ...
                                        <td scope="col" class="px-6 py-3 text-center">
                                            {{ @$siswa->sekolah_asal ?? 'Belum Di Lengkapi' }}
                                        </td>
                                        <td scope="col" class="px-6 py-3 text-center">
                                        ...
                                        <td scope="col" class="px-6 py-3 text-center">
                                            @if ($siswa->dataRegistrasi->status == '1')
                                                <span
                                                    class="px-2 py-1 bg-green-500 text-white font-medium rounded-lg">Lulus</span>
                                            @elseif($siswa->dataRegistrasi->status == '2')
                                                <span
                                                    class="px-2 py-1 bg-red-500 text-white font-medium rounded-lg">Tidak
                                                    Lulus</span>
                                            @else
                                                <span
                                                    class="px-2 py-1 bg-yellow-500 text-white font-medium rounded-lg">Belum
                                                    diproses</span>
                                            @endif
                                        <td scope="col" class="px-6 py-3 text-center">
