<x-app-layout>
            ...
            @else
                <div class="flex flex-col md:flex-row items-center md:items-start gap-5">
                    <div class="flex-1 text-white text-center md:text-left">
                        ...
                        <p class="mt-3 text-green-200 text-base">Pantau status pendaftaran kamu di sini dan lengkapi semua
                            berkas yang diperlukan.</p>
                    </div>
                    @if($calonSiswa && $calonSiswa->dataRegistrasi)
                        <div
                            class="shrink-0 bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl px-6 py-4 text-white text-center min-w-[180px]">
                            <p class="text-xs text-green-200 uppercase tracking-widest mb-1">Jalur Pendaftaran</p>
                            <p class="font-bold text-xl">{{ $calonSiswa->dataRegistrasi->jalur->nama_jalur ?? 'Belum Dipilih' }}
                            </p>
        ...
        <div class="container mx-auto px-4 py-6">
            <div class="flex items-center justify-center max-w-3xl mx-auto">
                ...
                <div class="flex flex-col items-center text-center flex-1">
                    ...
                    <p class="mt-2 text-sm font-semibold {{ $activeStep >= 1 ? 'text-tertiary' : 'text-gray-400' }}">
                        Daftar Diri</p>
                    <p class="text-xs {{ $activeStep >= 1 ? 'text-gray-500' : 'text-gray-300' }} mt-0.5">
                        {{ $daftarDiriDetail }}
                    </p>
                </div>

                {{-- Connector 1-2 --}}
                <div class="flex-1 h-0.5 mx-2 mb-7 rounded {{ $activeStep >= 2 ? 'bg-tertiary' : 'bg-gray-200' }}">
                </div>
                ...
                <div class="flex flex-col items-center text-center flex-1">
                    ...
                    <p class="mt-2 text-sm font-semibold {{ $activeStep >= 2 ? 'text-tertiary' : 'text-gray-400' }}">
                        Verifikasi Berkas</p>
                    <p class="text-xs {{ $activeStep >= 2 ? 'text-gray-500' : 'text-gray-300' }} mt-0.5">
                        {{ $verifikasiDetail }}
                    </p>
                </div>

                {{-- Connector 2-3 --}}
                <div class="flex-1 h-0.5 mx-2 mb-7 rounded {{ $activeStep >= 3 ? 'bg-tertiary' : 'bg-gray-200' }}">
                </div>
                ...
                <div class="flex flex-col items-center text-center flex-1">
                    ...
                    <p class="mt-2 text-sm font-semibold {{ $activeStep >= 3 ? 'text-tertiary' : 'text-gray-400' }}">Tes
                        & Wawancara</p>
                    <p class="text-xs {{ $activeStep >= 3 ? 'text-gray-500' : 'text-gray-300' }} mt-0.5">
                        {{ $tesWawancaraDetail }}
                    </p>
                </div>
                ...
                {{-- Connector 3-4 --}}
                <div
                    class="flex-1 h-0.5 mx-2 mb-7 rounded {{ in_array($status, [6, 7, 8]) ? 'bg-tertiary' : 'bg-gray-200' }}">
                </div>

                @php
                    $showPenetapan = false;
                    $idJalur = $calonSiswa?->dataRegistrasi?->id_jalur ?? null;

                    // Jalur Afirmasi
                    if ($idJalur !== null && $idJalur != 1) {
                ...
                <!-- <div class="flex flex-col items-center text-center flex-1">
                    ...
                    <p
                        class="mt-2 text-sm font-semibold {{ in_array($status, [6, 7, 8]) ? 'text-tertiary' : 'text-gray-400' }}">
                        Hasil Seleksi
                    </p>
                    @if ($showPenetapan)
    ...
    <div class="container mx-auto px-4 py-8 space-y-8">
        ...
        @if (($status < 3 || $isDaftarUlang && $showAfter3PM))
            <div
                class="relative overflow-hidden bg-gradient-to-r from-primary to-tertiary rounded-2xl shadow-lg p-6 flex flex-col md:flex-row items-center gap-5">
                <div class="absolute right-0 top-0 bottom-0 w-48 opacity-10 hidden md:block">
                    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <path
                            d="M37.9,-65.5C49.4,-58.8,59.3,-49.5,66.4,-38.1C73.5,-26.7,77.8,-13.4,76.4,-0.8C75.1,11.8,68.1,23.6,60.2,34.3C52.3,45,43.6,54.6,32.7,61.1C21.9,67.6,9,71,-2.9,75.3C-14.8,79.6,-29.6,84.9,-40.9,79.7C-52.1,74.5,-59.8,59,-64.3,44C-68.7,28.9,-70,14.4,-70.7,-0.4C-71.4,-15.3,-71.5,-30.5,-64.8,-42.1C-58.2,-53.6,-44.8,-61.5,-31.8,-67.2C-18.9,-72.9,-6.4,-76.4,4.8,-83.9C15.9,-91.4,26.4,-72.3,37.9,-65.5Z"
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
                    ...
                    <div class="p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="col-span-1">
                            ...
                            <p
                                class="font-semibold text-gray-800 text-sm bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                        ...
                        <div class="col-span-1">
                            ...
                            <p
                                class="font-semibold text-sm bg-gray-50 rounded-lg px-3 py-2 border border-gray-100 {{ $jadwalTesBQ ? 'text-gray-800' : 'text-amber-500' }}">
                        ...
                        <div class="col-span-1">
                            ...
                            <p
                                class="font-semibold text-sm bg-gray-50 rounded-lg px-3 py-2 border border-gray-100 {{ $jadwalTesJapres ? 'text-gray-800' : 'text-amber-500' }}">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-5 space-y-3">
                            ...
                            @foreach($pribadi as $label => $value)
                                <div
                                    class="flex items-start justify-between gap-3 py-2 {{ !$loop->last ? 'border-b border-gray-50' : '' }}">
                                    <span
                                        class="text-xs font-medium text-gray-400 uppercase tracking-wide shrink-0 w-32">{{ $label }}</span>
